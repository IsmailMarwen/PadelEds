...
import { CreatePlageHoraireComponent } from '../create-plage-horaire/create-plage-horaire.component';
import { UpdatePlageHoraireComponent } from '../update-plage-horaire/update-plage-horaire.component';
declare var window: any;
declare const Waypoint: any;
@Component({
  selector: 'app-plage-horaire',
  templateUrl: './plage-horaire.component.html',
  styleUrl: './plage-horaire.component.css'
})
export class PlageHoraireComponent {
...
dureeAct:string='';
  ...
  searchQuery: string = '';
  plageHoraires: any[] = [];
  originalplageHoraires: any[] = []; // Array to store original plageHoraires before filtering
...
applyThemeUpdate(theme: string): void {
  ...
  getplageHoraires(): void {
    const clubId = localStorage.getItem("idClub");
    if(clubId)
    this.service.getPlageHoraire(clubId).subscribe(plageHoraires => {
      this.plageHoraires = plageHoraires;
      this.originalplageHoraires = plageHoraires;
    });
    else {
  ...
  searchplageHoraires(event: any): void {
    const searchTerm = event.target.value.toLowerCase();
    this.plageHoraires = this.originalplageHoraires.filter(plageHoraire => {
      return Object.values(plageHoraire).some((value: any) => 
        String(value).toLowerCase().includes(searchTerm)
      );
    });
  }
  
  deleteplageHoraire(plageHoraireId: number): void {
    this.service.deletePlageHoraire(plageHoraireId).subscribe(() => {
      // Remove the deleted plageHoraire from the plageHoraires array
      this.plageHoraires = this.plageHoraires.filter(plageHoraire => plageHoraire.id !== plageHoraireId);
  
      // Fetch the updated list of plageHoraires from the backend
      this.getplageHoraires();
    }, error => {
      console.error('Error deleting plageHoraire:', error);
    });
  }
  

  showplageHoraireFormModal() {
    this.isModalVisible = true;
  }

  hideplageHoraireFormModal() {
  ...
  submitForm() {
    ...
    this.hideplageHoraireFormModal();
  }
  ...
  openLargeModalAdd() {
    const modalRef = this.modalService.open(CreatePlageHoraireComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.name = 'World';
    modalRef.result.then((result) => {
      if (result === 'saved') {
        this.getplageHoraires();
  ...
  openLargeModalEdit(plageHoraire: any) {
    const modalRef = this.modalService.open(UpdatePlageHoraireComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.plageHoraire = { ...plageHoraire }; // Pass a copy of the plageHoraire object
  
    modalRef.result.then((result) => {
      if (result === 'updated') {
        // Handle success, e.g., refresh the plageHoraire list
        this.getplageHoraires();
      }
    }).catch((error) => {
      console.log(error);
