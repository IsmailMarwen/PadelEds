...
@Component({
  ...
  styleUrl: './login-club.component.css'
})
export class LoginClubComponent implements OnInit {
          ...
          if(data.token!=""){
            setTimeout(() => {
              ...
              if(this.getUpdated()){
                ...
                this.router.navigateByUrl(this.adresseUrl + '/updatePassword');
              }
            }, 2000);
          }else{
            setTimeout(() => {
              this.preloadButton = false;
              this.toast.toast('Username ou Mot de passe Incorrecte.', ToastType.DANGER, 'Erreur', 5000);
            }, 2000);
          }
  ...
  goToContact(){
    this.router.navigateByUrl(this.adresseUrl + '/contact');
