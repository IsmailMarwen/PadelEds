...
applyThemeUpdate(theme: string): void {
  ...
  searchbanques(event: any): void {
    ...
    this.banques = this.originalbanques.filter(banque => {
      return Object.values(banque).some((value: any) => 
        String(value).toLowerCase().includes(searchTerm)
      );
    });
  }
  
  deletebanque(banqueId: number): void {
    this.service.deleteBanque(banqueId).subscribe(() => {
      // Remove the deleted banque from the banques array
      this.banques = this.banques.filter(banque => banque.id !== banqueId);
  
      // Fetch the updated list of banques from the backend
      this.getbanques();
    }, error => {
  ...
  openLargeModalAdd() {
    const modalRef = this.modalService.open(CreateBanqueComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.name = 'World';
    modalRef.result.then((result) => {
      if (result === 'saved') {
        this.getbanques();
  ...
  openLargeModalEdit(banque: any) {
    const modalRef = this.modalService.open(UpdateBanqueComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.banque = { ...banque }; // Pass a copy of the banque object
  
    modalRef.result.then((result) => {
      if (result === 'updated') {
        // Handle success, e.g., refresh the banque list
        this.getbanques();
      }
    }).catch((error) => {
      console.log(error);
