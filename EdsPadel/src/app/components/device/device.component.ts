...
centieme:any
  ...
  searchQuery: string = '';
  devises: any[] = [];
  originaldevises: any[] = []; // Array to store original devises before filtering
...
applyThemeUpdate(theme: string): void {
  ...
  searchdevises(event: any): void {
    ...
    this.devises = this.originaldevises.filter(devise => {
      return Object.values(devise).some((value: any) => 
        String(value).toLowerCase().includes(searchTerm)
      );
    });
  }
  
  deletedevise(deviseId: number): void {
    this.service.deletedevise(deviseId).subscribe(() => {
      // Remove the deleted devise from the devises array
      this.devises = this.devises.filter(devise => devise.id !== deviseId);
  
      // Fetch the updated list of devises from the backend
      this.getdevises();
    }, error => {
  ...
  openLargeModalAdd() {
    const modalRef = this.modalService.open(CreateDeviceComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.name = 'World';
    modalRef.result.then((result) => {
      if (result === 'saved') {
        this.getdevises();
  ...
  openLargeModalEdit(devise: any) {
    const modalRef = this.modalService.open(UpdateDeviceComponent, { size: 'xl', centered: true });
    modalRef.componentInstance.devise = { ...devise }; // Pass a copy of the devise object
  
    modalRef.result.then((result) => {
      if (result === 'updated') {
        // Handle success, e.g., refresh the devise list
        this.getdevises();
      }
    }).catch((error) => {
      console.log(error);
