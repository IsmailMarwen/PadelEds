...
export class UpdateActiviteComponent {
  @Input() activite: any = {
    libelle: '',
    couleur: ''
  }; // Assuming this receives existing activity details
  ...
  updateActivite() {
    ...
    this.service.updateActivite(this.activite).subscribe(
      response => {
        console.log('Activite updated successfully:', response);
        this.preload = false;
      ...
      error => {
        ...
        if (error.error === "Le libelle existe déjà") {
          this.showErrorMessage('libelle', "Le libelle existe déjà");
        }

        this.toast.toast('Une erreur s\'est produite lors de la mise à jour de l\'activité.', ToastType.DANGER, 'Erreur', 5000);
      }
    );
  }
